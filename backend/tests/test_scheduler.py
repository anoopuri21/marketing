...
def test_weekly_next_run_is_monday_9am_ist():
    ...
    assert nxt == datetime(2026, 9, 7, 3, 30, tzinfo=UTC)
...
def test_weekly_same_day_after_time_rolls_a_week():
    now = datetime(2026, 9, 7, 5, 0, tzinfo=UTC)  # 10:30 IST Monday
    nxt = compute_next_run(_sched(), after=now)
    assert nxt == datetime(2026, 9, 14, 3, 30, tzinfo=UTC)
